...
export default {
    ...
    async queryDate() {
      await db
        ...
        .then((doc) => {
          doc.forEach((doc) => {
            const obj = {};
            obj.beginTime = moment(doc.data().Date)
              .format("YYYY-MM-DD")
              .concat(" " + doc.data().BeginingTime);
            obj.endTime = moment(doc.data().Date)
              .format("YYYY-MM-DD")
              .concat(" " + doc.data().EndingTime);
            this.Check.push(obj);
          });
        });
    },
    async checkDateTime() {
      {
        for (let i = 0; i < this.Check.length; i++) {
          for (let j = 0; j < this.date.length; j++) {
            ...
            const timeAddEnd = moment(this.date[j])
              .format("YYYY-MM-DD")
              .concat(" " + this.endingTime);
            const checkTimeBetween = moment(timeAddBegin).isBetween(
              this.Check[i].beginTime,
              this.Check[i].endTime
            );
            const checkTimeBetweenEnd = moment(timeAddEnd).isBetween(
              this.Check[i].beginTime,
              this.Check[i].endTime
            );
            const checkBeginTime = moment(timeAddBegin).isSame(
              moment(this.Check[i].beginTime)
            );
            const checkEndTime = moment(timeAddEnd).isSame(
              moment(this.Check[i].endTime)
            );
  ...
  computed: {
    totalServicecharge() {
      ...
      let finalTime = 0;
      if (
        moment(this.beginingTime, "h:mm").isBefore(
          moment(this.endingTime, "h:mm")
        )
      ) {
        const totalTime = moment(this.endingTime, "h:mm").diff(
          moment(this.beginingTime, "h:mm"),
          "minutes"
        );
        if (totalTime % 60 !== 0) {
    ...
    timeCalculator() {
      ...
      if (
        ...
        this.quasarPlugin.notify({
          message: "Please select new time",
          color: "red",
        });
      }
    },
